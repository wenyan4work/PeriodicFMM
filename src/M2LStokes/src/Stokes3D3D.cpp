...
inline void GkernelEwald(const Eigen::Vector3d &rvec, Eigen::Matrix3d &Gsum) {
  ...
  if (r < 1e-14) {
    auto Gself =
        -4 * xi / sqrt(PI314) * Eigen::Matrix3d::Identity(); // the self term
    for (int i = -N; i < N + 1; i++) {
      for (int j = -N; j < N + 1; j++) {
        for (int k = -N; k < N + 1; k++) {
          if (i == 0 && j == 0 && k == 0) {
...
int main(int argc, char **argv) {
  ...
  const double pCenterEquiv[3] = {-(scaleEquiv - 1) / 2, -(scaleEquiv - 1) / 2,
                                  -(scaleEquiv - 1) / 2};
  const double pCenterCheck[3] = {-(scaleCheck - 1) / 2, -(scaleCheck - 1) / 2,
                                  -(scaleCheck - 1) / 2};
...
#pragma omp parallel for
  for (int k = 0; k < checkN; k++) {
    ...
    for (int l = 0; l < equivN; l++) {
      const Eigen::Vector3d Lpoint(pointLEquiv[3 * l], pointLEquiv[3 * l + 1],
                                   pointLEquiv[3 * l + 2]);
