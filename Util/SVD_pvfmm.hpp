...
#ifndef M2LLAPLACE_1D3D_SVD_PVFMM_HPP_
#define M2LLAPLACE_1D3D_SVD_PVFMM_HPP_
...
#pragma omp parallel for
  ...
  while (k0 < dim[1] - 1) { // Diagonalization
    ...
    { // Compute mu
      ...
      C[1][1] =
          S(n - 1, n - 1) * S(n - 1, n - 1) + S(n - 2, n - 1) * S(n - 2, n - 1);
...
template <class T> void pinv_pvfmm(T *M, int n1, int n2, T eps, T *M_) {
  ...
  svd(&JOBU, &JOBVT, &m, &n, &M[0], &m, &tS[0], &tU[0], &m, &tVT[0], &k, wsbuf,
      &wssize, &INFO);
  if (INFO != 0)
    std::cout << INFO << '\n';
  assert(INFO == 0);
...
void pinv(const Eigen::MatrixXd &Mat, Eigen::MatrixXd &MatPinvU,
