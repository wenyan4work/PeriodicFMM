...
#define SRC_BLK 500
        ...
        for (size_t sblk = 0; sblk < src_cnt_; sblk += SRC_BLK) {
            ...
            for (size_t t = 0; t < trg_cnt_; t += VecLen) {
                ...
                tv0 = add_intrin(mul_intrin(tv0, oofp), load_intrin<Vec_t>(&trg_value[1][t])); // gradient
                tv1 = add_intrin(mul_intrin(tv1, oofp), load_intrin<Vec_t>(&trg_value[2][t]));
                tv2 = add_intrin(mul_intrin(tv2, oofp), load_intrin<Vec_t>(&trg_value[3][t]));
...
#define LAP_KER_NWTN(nwtn)                                                     \
  ...
  generic_kernel<                                                              \
      Real_t, 3, 1,                                                            \
      laplace_dipolepotential_uKernel<Real_t, Vec_t, rsqrt_intrin##nwtn<Vec_t, Real_t>>>( \
      (Real_t *)r_src, src_cnt, (Real_t *)v_src, dof, (Real_t *)r_trg,         \
      trg_cnt, (Real_t *)v_trg, mem_mgr)
#define LAPLACE_KERNEL                                                         \
  LAP_KER_NWTN(0);                                                             \
...
#define SRC_BLK 500
        ...
        const Real_t OOFP = -1.0 / (4 * nwtn_scal * nwtn_scal * nwtn_scal * const_pi<Real_t>());
        const Real_t OOFP2 = -1.0
                / (4 * nwtn_scal * nwtn_scal * nwtn_scal * nwtn_scal * nwtn_scal * const_pi<Real_t>());
...
#define LAP_KER_NWTN(nwtn)                                                     \
  ...
  generic_kernel<Real_t, 3, 4,                                                 \
                 laplace_dipolepgrad_uKernel<Real_t, Vec_t,                        \
                                         rsqrt_intrin##nwtn<Vec_t, Real_t>>>(  \
      (Real_t *)r_src, src_cnt, (Real_t *)v_src, dof, (Real_t *)r_trg,         \
      trg_cnt, (Real_t *)v_trg, mem_mgr)
#define LAPLACE_KERNEL                                                         \
...
#undef LAPLACE_KERNEL
    ...
    template<class T> struct LaplaceCustomKernel {
        inline static const Kernel<T> &potentialGradient();
        inline static const Kernel<T> &dipoleGradient();
    };
    ...
    const Kernel<T> &LaplaceCustomKernel<T>::potentialGradient() {
        ...
        static Kernel<T> pgrad_ker = BuildKernel<T, laplace_grad<T, 1>>("laplace_pgrad", 3, std::pair<int, int>(1, 4),
                &potn_ker, &potn_ker, NULL, &potn_ker, &potn_ker, NULL, &potn_ker, NULL);
        return pgrad_ker;
    }

    template<class T> const Kernel<T> &LaplaceCustomKernel<T>::dipoleGradient() {
        static Kernel<T> potn_ker = BuildKernel<T, laplace_dipolepotential_poten<T, 1>>("laplace_dipolepotential", 3,
                std::pair<int, int>(3, 1)); // source dim, target dim
        static Kernel<T> pgrad_ker = BuildKernel<T, laplace_dipolepgrad_poten<T, 1>>("laplace_dipolepgrad", 3,
                std::pair<int, int>(3, 4), &potn_ker, &potn_ker, NULL, &potn_ker, &potn_ker, NULL, &potn_ker, NULL);
        return pgrad_ker;
    }

// two newton iterations for double type
    template<>
    const Kernel<double> &LaplaceCustomKernel<double>::potentialGradient() {
        typedef double T;
        static Kernel<T> potn_ker = BuildKernel<T, laplace_poten<T, 2>>("laplace", 3, std::pair<int, int>(1, 1));

        static Kernel<T> pgrad_ker = BuildKernel<T, laplace_pgrad<T, 2>>("laplace_pgrad", 3, std::pair<int, int>(1, 4),
                &potn_ker, &potn_ker, NULL, &potn_ker, &potn_ker, NULL, &potn_ker, NULL);
        return pgrad_ker;
    }

    template<>
    const Kernel<double> &LaplaceCustomKernel<double>::dipoleGradient() {
        typedef double T;
        static Kernel<T> potn_ker = BuildKernel<T, laplace_dipolepotential_poten<T, 2>>("laplace_dipolepotential", 3,
                std::pair<int, int>(3, 1)); // source dim, target dim
        static Kernel<T> pgrad_ker = BuildKernel<T, laplace_dipolepgrad_poten<T, 2>>("laplace_dipolepgrad", 3,
                std::pair<int, int>(3, 4), &potn_ker, &potn_ker, NULL, &potn_ker, &potn_ker, NULL, &potn_ker, NULL);
        return pgrad_ker;
    }
}
