...
void calcTrueValueFMM(std::vector<double> &trg_value_true, const std::vector<double> &trg_coord,
                      ...
                      const double shift, const RPYFMM_Wrapper::PAXIS pset, const int kernel) {
    std::cout << "***************************************************" << std::endl;
    std::cout << "Skip O(N^2) true value calculation for large system" << std::endl;
    std::cout << "Use FMM p=16 as 'true' value                       " << std::endl;
    std::cout << "***************************************************" << std::endl;
    RPYFMM_Wrapper myFMM(16, 1000, 0, pset, kernel == 0 ? RPYFMM_Wrapper::KERNEL::G : RPYFMM_Wrapper::KERNEL::GRPY);
    myFMM.FMM_SetBox(shift, shift + box, shift, shift + box, shift, shift + box);
    myFMM.FMM_UpdateTree(src_coord, trg_coord);
    if (kernel == 0) {
        myFMM.FMM_EvaluateG(trg_value_true, trg_coord.size() / 3, src_value);
    } else if (kernel == 1) {
        myFMM.FMM_EvaluateRPY(trg_value_true, trg_coord.size() / 3, src_value);
    }
    return;
}
...
#pragma omp parallel for
    for (int t = 0; t < trg_coord.size() / 3; t++) {
        ...
        target[2] -= shift;
        target *= (1 / box);
        if (pset == RPYFMM_Wrapper::PAXIS::PX || pset == RPYFMM_Wrapper::PAXIS::PYZ) {
            Eigen::Vector3d temp = target;
            target[0] = temp[1];
            target[1] = temp[2];
            target[2] = temp[0];
        } else if (pset == RPYFMM_Wrapper::PAXIS::PY || pset == RPYFMM_Wrapper::PAXIS::PXZ) {
            Eigen::Vector3d temp = target;
            target[0] = temp[2];
            target[1] = temp[0];
            target[2] = temp[1];
        ...
        for (int s = 0; s < src_coord.size() / 3; s++) {
            ...
            Eigen::Matrix3d G;
            if (kernel == 0) {
                if (pset == RPYFMM_Wrapper::PAXIS::PXYZ) {
                    GkernelEwald3D(rst, G);
                } else if (pset == RPYFMM_Wrapper::PAXIS::PXY || pset == RPYFMM_Wrapper::PAXIS::PXZ ||
                           pset == RPYFMM_Wrapper::PAXIS::PYZ) {
                    GkernelEwald2D(rst, G); // default box = 1
                } else if (pset == RPYFMM_Wrapper::PAXIS::PX || pset == RPYFMM_Wrapper::PAXIS::PY ||
                           pset == RPYFMM_Wrapper::PAXIS::PZ) {
                    Gkernel1D(rst, G); // default box =1
                } else if (pset == RPYFMM_Wrapper::PAXIS::NONE) {
                    Gkernel(rst, Eigen::Vector3d(0, 0, 0), G);
                }
                targetValue += (G * sourceValue) / (8 * PI314);
            } else if (kernel == 1) {
                if (pset == RPYFMM_Wrapper::PAXIS::PXYZ) {
                    GkernelRPYEwald3D(rst, G);
                } else if (pset == RPYFMM_Wrapper::PAXIS::PXY || pset == RPYFMM_Wrapper::PAXIS::PXZ ||
                           pset == RPYFMM_Wrapper::PAXIS::PYZ) {
                    printf("WARNING: EWALD 2D for RPY NOT Implemented yet\n");
                } else if (pset == RPYFMM_Wrapper::PAXIS::PX || pset == RPYFMM_Wrapper::PAXIS::PY ||
                           pset == RPYFMM_Wrapper::PAXIS::PZ) {
                    printf("WARNING: M2L data for 1D RPY NOT Implemented\n");
                    exit(1);
                } else if (pset == RPYFMM_Wrapper::PAXIS::NONE) {
                    GkernelRPY(rst, Eigen::Vector3d(0, 0, 0), G);
                }
                targetValue += (G * sourceValue) / (4 * PI314);
            }
        }
        ...
        if (kernel == 0) {
            ...
            const double box3 = box * box * box;
            if (pset == RPYFMM_Wrapper::PAXIS::PX || pset == RPYFMM_Wrapper::PAXIS::PYZ) {
                trg_value_true[3 * t] = targetValue[2] / box3;
                trg_value_true[3 * t + 1] = targetValue[0] / box3;
                trg_value_true[3 * t + 2] = targetValue[1] / box3;
            } else if (pset == RPYFMM_Wrapper::PAXIS::PY || pset == RPYFMM_Wrapper::PAXIS::PXZ) {
                trg_value_true[3 * t] = targetValue[1] / box3;
                trg_value_true[3 * t + 1] = targetValue[2] / box3;
                trg_value_true[3 * t + 2] = targetValue[0] / box3;
            } else {
                trg_value_true[3 * t] = targetValue[0] / box3;
                trg_value_true[3 * t + 1] = targetValue[1] / box3;
                trg_value_true[3 * t + 2] = targetValue[2] / box3;
            }
        }
    }
...
void initPts(std::vector<double> &src_coord, std::vector<double> &src_value, std::vector<double> &trg_coord,
    ...
    RPYFMM_Wrapper::PAXIS pset;
    int pinput = parser.get<int>("P");
    switch (pinput) {
    case 0:
        pset = RPYFMM_Wrapper::PAXIS::NONE;
        break;
    case 1:
        pset = RPYFMM_Wrapper::PAXIS::PX;
        break;
    case 2:
        pset = RPYFMM_Wrapper::PAXIS::PY;
        break;
    case 3:
        pset = RPYFMM_Wrapper::PAXIS::PZ;
        break;
    case 4:
        pset = RPYFMM_Wrapper::PAXIS::PXY;
        break;
    case 5:
        pset = RPYFMM_Wrapper::PAXIS::PXZ;
        break;
    case 6:
        pset = RPYFMM_Wrapper::PAXIS::PYZ;
        break;
    case 7:
        pset = RPYFMM_Wrapper::PAXIS::PXYZ;
        break;
    }
    ...
    if (pset == RPYFMM_Wrapper::PAXIS::PX || pset == RPYFMM_Wrapper::PAXIS::PY || pset == RPYFMM_Wrapper::PAXIS::PZ ||
        pset == RPYFMM_Wrapper::PAXIS::PXY || pset == RPYFMM_Wrapper::PAXIS::PYZ ||
        pset == RPYFMM_Wrapper::PAXIS::PXZ) {
...
void testFMM(std::vector<double> &trg_value, std::vector<double> &trg_coord, std::vector<double> &src_value,
             ...
             RPYFMM_Wrapper::PAXIS pset, const int kernel) {
    int myRank;
    int nProcs;
    MPI_Comm_rank(MPI_COMM_WORLD, &myRank);
    MPI_Comm_size(MPI_COMM_WORLD, &nProcs);
    ...
    RPYFMM_Wrapper myFMM(p, 1000, 0, pset, kernel == 0 ? RPYFMM_Wrapper::KERNEL::G : RPYFMM_Wrapper::KERNEL::GRPY);
    myFMM.FMM_SetBox(shift, shift + box, shift, shift + box, shift, shift + box);
    ...
    if (kernel == 0) {
        myFMM.FMM_EvaluateG(trg_value, trg_coord.size() / 3, src_noise);
    } else if (kernel == 1) {
        myFMM.FMM_EvaluateRPY(trg_value, trg_coord.size() / 3, src_noise);
    }

    myFMM.FMM_DataClear();
    if (kernel == 0) {
        myFMM.FMM_EvaluateG(trg_value, trg_coord.size() / 3, src_value);
    } else if (kernel == 1) {
        myFMM.FMM_EvaluateRPY(trg_value, trg_coord.size() / 3, src_value);
    }
...
void configure_parser(cli::Parser &parser) {
    ...
    parser.set_optional<int>("K", "kernel", 0, "0 for Stokes G, 1 for RPY Q");
}
...
int main(int argc, char **argv) {
    ...
    const double box = parser.get<double>("B");
    const double shift = parser.get<double>("M");
    const int kernel = parser.get<int>("K");
    ...
    if (myRank == 0) {
        ...
        if (src_coord.size() * trg_coord.size() < 3 * 1e7) {
            trueN2 = 1;
            calcTrueValueN2(trg_true, trg_coord, src_value, src_coord, box, shift, pset, kernel);
        }
        MPI_Bcast(&trueN2, 1, MPI_INT, 0, MPI_COMM_WORLD);
        if (trueN2 == 0) {
            calcTrueValueFMM(trg_true, trg_coord, src_value, src_coord, box, shift, pset, kernel);
        }
    } else {
        int trueN2 = 0;
        MPI_Bcast(&trueN2, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    for (int p = 6; p <= 16; p += 2) {
        testFMM(trg_value, trg_coord, src_value, src_coord, trg_true, p, box, shift, pset, kernel);
